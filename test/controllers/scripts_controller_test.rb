...
class ScriptsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create script" do
    assert_difference('Script.count') do
      post scripts_url, params: { script: { command: @script.command, parameters_id: @script.parameters_id } }
    end
  ...
  test "should update script" do
    patch script_url(@script), params: { script: { command: @script.command, parameters_id: @script.parameters_id } }
